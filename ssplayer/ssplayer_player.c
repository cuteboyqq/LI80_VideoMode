...
#if 0 // for decode video packet to mark
...
static player_stat_t *player_init(const char *p_input_file)
{
    ...
    // init sdk sys
    // CheckFuncResult(ss_sys_Init()); //for decode video packet to mark

    av_init_packet(&flush_pkt);
    flush_pkt.data = (uint8_t *)&flush_pkt;

    packet_queue_put(&is->video_pkt_queue, &flush_pkt);
    // packet_queue_put(&is->audio_pkt_queue, &flush_pkt); //for decode video packet to mark

    CheckFuncResult(pthread_cond_init(&is->continue_read_thread, NULL));

    init_clock(&is->video_clk, &is->video_pkt_queue.serial);
    // init_clock(&is->audio_clk, &is->audio_pkt_queue.serial);//for decode video packet to mark
...
static int ss_module_Deinit(player_stat_t *is)
{
    if (is->video_idx >= 0)
    {
        // sstar_disable_display(); //for decode video packet to mark
    }
    ...
    // deinit ss sys
    // CheckFuncResult(MI_SYS_Exit()); //for decode video packet to mark
    return SUCCESS;
}
...
int player_running(const char *p_input_file, char *type)
{
    ...
    open_video(is);
    // ADAS.run...
    // open_audio(is);//for decode video packet to mark
...
// Enqueue an AVFrame pointer into the queue
...
// Dequeue an AVFrame pointer from the queue
