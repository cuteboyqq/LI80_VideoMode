...
#include "ssplayer_player.h" //Alister add 2024-03-14
// WNC-Yolo-Adas star
// #include "dla_base.h"
...
static void *Adas_Task_VideoMode(void *args);
...
static MI_S32 ADAS_Run(ADAS_Context *adas_ctx)
{
    int  ret;
    int  MODE    = 1;
    char video[] = "path/to/the/video";
    if (MODE == 1)
    {
        // Do Video Mode
        player_running(video, 0);
...
static MI_S32 notify_event_to_iox(time_t ra_time, int event_type)
{
    int         iRet   = -1;
    int         iMsqId = -1;
    time_t      dwTime;
...
static S32 ADAS_GetSrcImage(ADAS_Context *adas_ctx, U8 **source_image, MI_SYS_BUF_HANDLE *hHandle)
{
    ...
    if (MI_SUCCESS == s32Ret)
    {
        ...
        g_IpuIntfObject->_scaleToModelSize(
            (MI_SYS_BufInfo_t *)&stBufInfo); // Entry Point at the end //wnc_add because need to new a object first
        // m_logger->debug("Finished converting buffered image to cv::Mat");

        if (g_IpuIntfObject->m_frameIdx_wnc % g_IpuIntfObject->m_frameStep_wnc == 0)
        {
            g_IpuIntfObject->logInfo("", "", LOG_DEBUG);
            g_IpuIntfObject->logInfo("========================================", "", LOG_DEBUG);
            g_IpuIntfObject->logInfo("Frame Index: " + to_string(g_IpuIntfObject->m_frameIdx_wnc), "", LOG_DEBUG);
            g_IpuIntfObject->logInfo("========================================", "", LOG_DEBUG);

            // Update YOLO-ADAS frame buffer
            g_IpuIntfObject->m_yoloADAS->updateInputFrame(g_IpuIntfObject->m_img);

            // Get last prediction
            YOLOADAS_Prediction pred           = {};
            int                 predBufferSize = g_IpuIntfObject->m_yoloADAS->getLastestPrediction(pred);
            if (predBufferSize > 0)
            {
                // Start doing post processing ...
                g_IpuIntfObject->m_yoloADAS_PostProc->updatePredictionBuffer(pred);

                int resultBufferSize =
                    g_IpuIntfObject->m_yoloADAS_PostProc->getLastestResult(g_IpuIntfObject->m_procResult);

                if (resultBufferSize == 0)
                {
                    if (MI_SUCCESS != MI_SYS_ChnOutputPortPutBuf(*hHandle)) // wnc modify to hHandle from stBufHandle
                    {
                        // m_logger->error("[OD_Tracking] MI_SYS_ChnOutputPortPutBuf error");
                        g_IpuIntfObject->logInfo("MI_SYS_ChnOutputPortPutBuf error!", __FUNCTION__, LOG_ERR);
                        // ret = ADAS_FAILURE;
                        // exit(1);
                    }
                    *source_image = 0;
                ...
                if (!g_IpuIntfObject->_laneLineDetection())
                {
                    // m_logger->warn("Detect lane lines failed ...");
                    g_IpuIntfObject->logInfo("Detecting lane lines failed!", __FUNCTION__, LOG_WARNING);
                    FRAME_SUCCESS = ADAS_FAILURE;
                }

                // Get Detected Bounding Boxes
                if (!g_IpuIntfObject->_objectDetection())
                {
                    // m_logger->warn("Detect objects failed ...");
                    g_IpuIntfObject->logInfo("Detecting objects failed!", __FUNCTION__, LOG_WARNING);
                    FRAME_SUCCESS = ADAS_FAILURE;
                }

                // Lane Line Calibration Using Vehicle's Information
                if (!g_IpuIntfObject->_laneLineCalibration())
                {
                    // m_logger->warn("Calibrate lane lines failed ...");
                    g_IpuIntfObject->logInfo("Calibrating lane lines failed!", __FUNCTION__, LOG_WARNING);
                    FRAME_SUCCESS = ADAS_FAILURE;
                }

                // Filter some objects out to reduce CPU loading
                if (!g_IpuIntfObject->_objectFiltering())
                {
                    // m_logger->warn("Filter objects failed ...");
                    g_IpuIntfObject->logInfo("Filtering objects failed!", __FUNCTION__, LOG_WARNING);
                    FRAME_SUCCESS = ADAS_FAILURE;
                }

                // Object Tracking
                if (!g_IpuIntfObject->_objectTracking())
                {
                    // m_logger->warn("Track objects failed ...");
                    g_IpuIntfObject->logInfo("Tracking objects failed!", __FUNCTION__, LOG_WARNING);
                    FRAME_SUCCESS = ADAS_FAILURE;
                }

                // Detect Lane Departure Event
                if (!g_IpuIntfObject->_laneDepartureDetection())
                {
                    // m_logger->warn("Detect lane departure event failed ...");
                    g_IpuIntfObject->logInfo("Detecting lane departure event failed!", __FUNCTION__, LOG_WARNING);
                    FRAME_SUCCESS = ADAS_FAILURE;
                }

                // Forward Collision Warning
                if (!g_IpuIntfObject->_forwardCollisionDetection())
                {
                    // m_logger->warn("Detect forward collision event failed ...");
                    g_IpuIntfObject->logInfo("Detecting forward collision event failed!", __FUNCTION__, LOG_WARNING);
                    FRAME_SUCCESS = ADAS_FAILURE;
                }

                // End of ADAS Tasks
                g_IpuIntfObject->m_yoloADAS_PostProc->removeFirstResult();
                ...
                if (g_IpuIntfObject->m_dbg_saveLogs)
                    g_IpuIntfObject->_saveDetectionResults();
                ...
                if (g_IpuIntfObject->m_estimateTime)
                {
                    ...
                    //                    / (1000.0 * 1000));
                    g_IpuIntfObject->logInfo("", "", LOG_INFO);
                    g_IpuIntfObject->logInfo(
                        "Processing Time: "
        ...
        if (g_IpuIntfObject->m_dsp_results && g_IpuIntfObject->m_dbg_saveImages && FRAME_SUCCESS == ADAS_SUCCESS)
        ...
        std::string json_log_str = g_IpuIntfObject->m_json_log->JsonLogString_2(
            adasResult, g_IpuIntfObject->m_humanBBoxList, g_IpuIntfObject->m_riderBBoxList,
            g_IpuIntfObject->m_vehicleBBoxList, g_IpuIntfObject->m_roadSignBBoxList,
            g_IpuIntfObject->m_stopSignBBoxList, g_IpuIntfObject->m_trackedObjList, g_IpuIntfObject->m_frameIdx_wnc);
        ...
        if (adasResult.eventType != ADAS_EVENT_NORMAL)
        {
            ...
            {
                if (g_IpuIntfObject->m_dbg_notify_fcw || g_IpuIntfObject->m_dbg_notify_ldw)
                {
                    MI_S8 audioparam[64];
                    audioparam[0] = 1;
                ...
                // m_logger->info("Detect LDW Event!");
                // m_logger->info("Detect FCW Event!");
                g_IpuIntfObject->logInfo("Detect LDW + FCW Event", "", LOG_DEBUG);
                if ((now_time - g_last_notify) >= notify_interval)
                {
                    syslog(LOG_INFO, "%s  %d \n", __func__, __LINE__);
                    process_event_record(now_time, iInterval, MSG_CMD_EVENT);
                    notify_event_to_iox(now_time, MSG_CMD_EVENT_LDW);
                    usleep(100);
                    notify_event_to_iox(now_time, MSG_CMD_EVENT_FCW);
                }
            }
            else if (adasResult.eventType == ADAS_EVENT_LDW_LEFT)
            {
                ...
                // m_logger->info("Detect LDW Left Event!");
                g_IpuIntfObject->logInfo("Detect LDW Left", "", LOG_DEBUG);
                if ((now_time - g_last_notify) >= notify_interval)
                {
            ...
            else if (adasResult.eventType == ADAS_EVENT_LDW_RIGHT)
            {
                ...
                // m_logger->info("Detect LDW Right Event!");
                g_IpuIntfObject->logInfo("Detect LDW Right", "", LOG_DEBUG);
                if ((now_time - g_last_notify) >= notify_interval)
                {
                    syslog(LOG_INFO, "%s  %d now_time %ld\n", __func__, __LINE__, now_time);
                    process_event_record(now_time, iInterval, MSG_CMD_EVENT_LDW);
            ...
            else if (adasResult.eventType == ADAS_EVENT_FCW)
            {
                ...
                // m_logger->info("Detect FCW Event!");
                g_IpuIntfObject->logInfo("Detect FCW", "", LOG_DEBUG);
                if ((now_time - g_last_notify) >= notify_interval)
                {
                    syslog(LOG_INFO, "%s  %d \n", __func__, __LINE__);
        ...
        // g_IpuIntfObject->IpuShowOutput(adasResult);
    ...
    else
    {
        ...
        FRAME_SUCCESS = ADAS_FAILURE; // 	[WNC] add WNC YOLOADAS service
    }
...
static S32 ADAS_GetSrcImage_FromVideo(ADAS_Context *adas_ctx, U8 **source_image, MI_SYS_BUF_HANDLE *hHandle)
{
    MI_S32           s32Ret = 0;
    MI_SYS_BufInfo_t stBufInfo;
    ...
    if (MI_SUCCESS == s32Ret)
    {
        ...
        // m_logger->debug("Start converting buffered image to cv::Mat");
        g_IpuIntfObject->_scaleToModelSize_FromVideo(
            (MI_SYS_BufInfo_t *)&stBufInfo); // Entry Point at the end //wnc_add because need to new a object first
        // m_logger->debug("Finished converting buffered image to cv::Mat");
...
// Test only end !
    ...
    while (adas_ctx->thread_param.thread_exit == FALSE)
    {
        ...
        else
        {
            if (FD_ISSET(s32Fd, &read_fds))
            {
                ret = ADAS_GetSrcImage_FromVideo(adas_ctx, &source_image, &hHandle); // WNC YOLO-ADAS entry point
// total_frame++;
#if dis_WNC_ADAS
                if (ret >= 0)
                {
